...
function bundle(expression) {
  ...
  try {
    ...
    return execFileSync('eyg', ['run', source], { encoding: 'utf8' })
      .trim().split('\n').map(line => JSON.parse(line)).map(f => file(f.path, f.media_type, f.text));
...
test('content-addressable carousel navigates and wraps images', async ({ page }) => {
  ...
  const frame = await (await agent(page)).show(files);
...
test('carousel empty and singleton states disable navigation', async ({ page }) => {
  ...
  const empty = await overlay.show(bundle('let v=import "./eyg_packages/artifact_viewers/carousel.eyg" v.render({title:"Empty",images:[]})'), 'empty');
  await expect(empty.locator('#empty')).toBeVisible();
  await expect(empty.getByRole('button', { name: 'Next image' })).toBeDisabled();
  const single = await overlay.show(bundle('let v=import "./eyg_packages/artifact_viewers/carousel.eyg" v.render({title:"One",images:[{caption:"Only",media_type:"image/svg+xml",content:!string_to_binary("<svg/>")}]})'), 'single');
...
test('Bluesky viewer renders supported lexicons as text and handles invalid feeds', async ({ page }) => {
  ...
  const frame = await overlay.show(bundle(expression), 'posts');
  ...
  const invalid = await overlay.show(bundle('let viewer=import "./eyg_packages/artifact_viewers/bluesky.eyg" viewer.render({title:"Posts",feed:"not json",retrieved_at:"now"})'), 'invalid');
...
test('Bluesky viewer handles empty feeds, wrong schemas and attachment notices', async ({ page }) => {
  ...
  let frame = await overlay.show(render({ feed: [] }), 'empty');
  await expect(frame.locator('#posts')).toHaveText('No supported posts in this feed.');
  frame = await overlay.show(render({ other: [] }), 'schema');
  await expect(frame.locator('#posts')).toContainText('Expected app.bsky.feed.getAuthorFeed response');
  frame = await overlay.show(render({ feed: [{ reason: {}, post: { record: { $type: 'app.bsky.feed.post', text: 'A photo', embed: {} } } }] }), 'attachment');

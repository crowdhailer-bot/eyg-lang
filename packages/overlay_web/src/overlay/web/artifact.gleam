...
/// Perform an artifact effect, returning the value a program resumes with.
pub fn perform(store: Store, effect: Effect) -> #(Store, v.Value(a, b)) {
  case effect {
    Save(name, bundle) ->
      case save(store, name, bundle) {
        Ok(#(store, version)) -> #(store, v.ok(v.Integer(version)))
        Error(reason) -> #(store, v.error(v.String(reason)))
      }
    Show(placement) ->
      case show(store, placement) {

//// Drawing a projection as a block of code, and telling which term was
//// clicked.
////
//// `render` and `frame` know how to turn code into elements; this is the last
//// step, where the focus is highlighted and the whole thing becomes something
//// a person can click on. Every application showing a structural editor wants
//// exactly this, so it lives here rather than in any one of them.
...
/// A projection, with the errors from an analysis marked and the focus
/// highlighted, that reports the path of whatever is clicked.
pub fn code(
  projection: p.Projection,
  analysis: infer.Analysis(List(Int)),
  user_clicked_code: fn(List(Int)) -> message,
) -> element.Element(message) {
  h.pre(
    [
      a.class("language-eyg"),
      a.styles(code_area_styles),
      event.on("click", click_decoder(user_clicked_code)),
    ],
...
pub fn render_projection(
  ...
  case focus, zoom {
    ...
    _, _ -> {
      ...
      let frame = render.projection_frame(projection, render.Statements, errors)
...
/// Every rendered term carries the path to itself in a `rev` data attribute,
/// reversed. A click anywhere reads the nearest one and hands back the path.
pub fn click_decoder(
  user_clicked_code: fn(List(Int)) -> message,

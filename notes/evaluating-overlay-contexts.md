...
The pure Fibonacci entrypoint deliberately stops before a session: one provider
request and one value check. Its guides are inline because effects are forbidden.
It shares Overlay's introductory instructions and run tool; session evals below
reuse the complete state machine and its prompt.

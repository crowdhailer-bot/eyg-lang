...
## Start with one pure program

The smallest eval makes one real model request using Overlay's shared
instructions and its `run` tool. It supplies the guides inline, type-checks the
returned program with no effects or imports, and compares its value to the
expected value. It has no effect handlers or judge.
...
This asks for the sum of the first 20 Fibonacci numbers, starting with `1, 1`.
The expected integer is `17710`. The [live results](../packages/overlay_eval/evidence/fibonacci.md)
keep both the failed and successful attempts.

The next level runs the same task through Overlay's full session, allowing it
to fetch guides and correct programs from tool feedback:

```sh
gleam run -m overlay/eval -- validate suites/fibonacci.eyg
OLLAMA_API_KEY=... gleam run -m overlay/eval -- run suites/fibonacci.eyg --model ollama:gpt-oss:120b
```

Add fixtures, contexts and more checks only when the task needs them. The
single-request pure command remains available throughout.
